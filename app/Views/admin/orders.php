...
<script>
    $(document).ready(function() {
        ...
        $("#addBtn").click(function() {
            $.post("<?= site_url("admin/orders/new") ?>", {
                id: $("#id").val(),
        ...
        function showdata(d) {
            ...
            $.each(d, function(index, row) {
                ...
                $html += `<td >${row.id}</td>`;
                $html += `<td class='sub'>${row.customer_name}</td>`;
                $html += `<td class='cls'>${row.payment}</td>`;
                $html += `<td class='img '>${row.tranxid}</td>`;
                $html += `<td class='q'>${row.total}</td>`;
                $html += `<td class='p'>${row.comment}</td>`;
                $html += `<td class='d'>${row.status}</td>`;
        ...
        function loaddata() {
            $.getJSON("<?= base_url(); ?>admin/orders/all", function(data) {
                showdata(data);
            });
            clearform()
        }
        ...
        $("#maindata").on("click", ".deleteBtn", function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    //           
                    // ajax            
                    $.post("<?= site_url("admin/orders/delete") ?>", {
                        'id': $id,
                        'action': "delete"
                    }, function(d) {
                        if (d.success) {
